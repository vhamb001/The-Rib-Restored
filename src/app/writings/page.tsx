    ...
    <div className="bg-white min-h-screen">
      ...
      <section className="py-10 sm:py-14 lg:py-20">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-6xl flex flex-col lg:flex-row gap-8 sm:gap-10 lg:gap-16">
          ...
          {/* Posts */}
          <div className="flex-1 space-y-4 sm:space-y-6 stagger-children">
            {writings.map((writing) => {
              ...
              return (
                <Link
                  ...
                  className="group flex flex-col sm:flex-row gap-4 sm:gap-5 p-4 sm:p-5 lg:p-6 rounded-sm border border-transparent hover:border-gray-100 hover:bg-[var(--color-warm-white)] hover:shadow-sm transition-all duration-500"
                >
                  <div className="relative w-full sm:w-40 md:w-48 shrink-0 aspect-[4/3] rounded-sm overflow-hidden bg-[var(--color-ivory)]">
                  ...
                  <div className="flex flex-col justify-center flex-grow min-w-0">
                    <div className="flex items-center gap-2 sm:gap-3 mb-1.5 sm:mb-2">
                      <span className="text-[0.55rem] sm:text-[0.6rem] font-bold tracking-[0.2em] text-[var(--color-gold)] uppercase">{writing.category}</span>
                      <span className="w-1 h-1 rounded-full bg-gray-300" />
                      <span className="text-[0.65rem] sm:text-xs text-gray-400">{date}</span>
                    </div>
                    <h3 className="font-heading text-lg sm:text-xl lg:text-2xl font-bold text-[var(--color-royal-deep)] mb-1.5 sm:mb-2 group-hover:text-[var(--color-gold)] transition-colors duration-300 leading-tight">
                      {writing.title}
                    </h3>
                    <p className="text-gray-500 text-sm leading-[1.7] line-clamp-2 mb-2 sm:mb-3">{writing.excerpt}</p>
